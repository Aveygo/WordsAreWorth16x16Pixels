...
class PACINGV8TINY(nn.Module):
    def __init__(self, seq_len=1024, n_embd=768, vocab_size=50257, in_channels=3):
        super().__init__()
        self.in_channels = in_channels

        self.patch_num = int((seq_len)**(1/2))
        self.patch_size = int((n_embd//in_channels)**(1/2))

        self.ln_f = nn.LayerNorm(n_embd)
        self.wte = nn.Embedding(vocab_size, n_embd)
        self.convnext = ConvNeXtV2(in_channels, vocab_size, depths=[2, 2, 4, 2], dims=[32, 64, 256, n_embd])
